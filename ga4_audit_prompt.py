"""
SEMAI GA4 Deep Audit Prompt v1.5.
...
CHANGELOG from v1.4 (external review, adopted):
- R4 corrected. form_submit / form_start (or any step-to-step ratio
  built purely from the Events report, with no input 13) is an
  event-volume proxy ratio, not a funnel completion rate: the events
  counted at each step may come from different users, sessions, or
  sequences, so nothing proves the same users completed the stages in
  order. When input 13 is absent, these ratios must be labeled as
  event-volume proxy ratios, and sequential stage conversion must be
  stated as Blocked, not implied by the ratio.
- R8 corrected. Input 13 (Funnel Exploration) being present does not
  by itself enable page-level diagnosis. A Funnel Exploration broken
  down only by Device and New/Returning supports R9, but cannot say
  which pages caused a drop-off. R8 now requires that input 13 either
  be segmented/broken down by landing page or page path, or be built
  as separate funnels with page-scoped step conditions, before
  page-level diagnosis is treated as unblocked. Otherwise, sitewide
  funnel diagnosis remains available, but page-level diagnosis is
  explicitly Blocked.
- Executive Summary requirement made conditional. "The query family
  associated with the best-converting landing pages" can only be
  reported when page-level conversion data is actually supplied (i.e.
  R8's page-level diagnosis is not Blocked). Otherwise the Executive
  Summary states "Blocked: no page-level conversion data available"
  for that claim, and reports the strongest query family by CTR
  separately, clearly labeled as a different, CTR-based measure, not
  a conversion measure.
- Input 16 corrected. "A GA4 Explore table broken down by User ID or
  Client ID" is not a dependable standard export for most properties
  and was too weak a stated requirement for something R12 depends on.
  The primary requirement is now a BigQuery export of GA4 event-level
  data (user_pseudo_id, session identifiers such as ga_session_id,
  event_timestamp, and the relevant event-name predicates), which can
  actually support a row-level join. A GA4 Explore User ID breakdown
  may supplement this where available, but is not treated as an
  equivalent standalone source.

Design decisions considered from this review and intentionally not
expanded further:
- The review's fixes were adopted as stated; none were partially
  adopted or reworded down in this pass. No new open items are logged
  here beyond what is already tracked in v1.3 and v1.4's changelogs
  (the addressable-volume composite score, and full performance-
  marketing inputs, both still intentionally out of scope).
"""
...
3. GA4 Landing page report: sessions, active users, new users, avg engagement time. This report has no conversion or key-event dimension; it cannot by itself support a page-level conversion claim (see Executive Summary rule).
...
State which tier rule was applied to each finding, not just the resulting tier, so the basis is auditable. Kill criteria may only fire on High-tier baselines, or on Anecdotal signals repeated across two consecutive periods. Anecdotal baselines get 30-day validation windows minimum, never 14.

### R7a. Defensible targets only
...
Input 14 (Page x Event free-form) may be used only as a cross-check against a finding already established from a page-segmented input 13, e.g. to sanity-check that an event count is in the expected range. It must never be the sole basis of an R8 finding, because it cannot establish sequence.
...
- Observed Volume: the volume behind the finding it addresses (sessions, users, or events), taken from the finding itself, never estimated, and never stated or implied as a dollar figure or revenue unless revenue data or a CRM join (R11) is actually present.
...
GA4 data, at any level of granularity (sitewide, page-level, device-level, cohort-level), can establish WHERE a drop-off happens and HOW MUCH volume it represents. It cannot establish WHY without session replay, user testing, or equivalent qualitative data. Every finding produced under R8 or R9 must be labeled as a WHERE finding. If session replay or equivalent qualitative input is not supplied, do not propose a specific root cause (e.g. "the form has too many fields") as a finding; propose it only as a labeled Hypothesis for validation, and name the validation method (e.g. "session replay on this page" or "a 5-user moderated test"). Revenue outcomes (registration to paying customer) additionally require a CRM or product-data join; if that join is not supplied, state it as Blocked in the same way as any other missing input, not silently omitted.
...
- Every rule R1 through R12 (including R7a) has a visible trace in the output: either the computation appears somewhere in the report, or the report explicitly states why it is Blocked (missing input) for this run. A rule that is neither applied nor marked Blocked is a defect, not a judgment call, and must be fixed before delivery.
- Every input in the INPUTS list (1-16) has a stated status (Present with row count, or Absent) in Input Validation, including the GA4 configuration checklist items and, if input 13 is present, its funnel configuration disclosure and its page-level-segmentation status.
- Every section in the Output Schema (1-17) appears, in order, with real content or an explicit Blocked statement, including Section AF, which is mandatory in structure even when its content is "Blocked: geography absent or not materially relevant." No section is silently dropped, merged into another without disclosure, or reordered without a note in Deviations.
...
- No "Expected" or "Kill if" cell contains an invented target; each is either sourced (R7a) or marked Blocked.
...
2. **Input Validation**: file/header consistency, row counts, truncation flags, the GA4 configuration checklist, the Funnel Exploration configuration and page-level-segmentation disclosure (if input 13 is present), and a present/absent table against every input in the INPUTS list, including 12-16, with what each absence blocks.
...
4. **Section X, Baseline Metrics Snapshot**: table with Cluster | Metric | Actual Value | Source | Evidence Volume Tier (state which R7 rule was applied). Include blended, branded, non-branded, each content cluster with its R6 engagement benchmark, sitewide sessions/users, and conversion capture status.
...
6. **Section Z, Build These Pages Next**: table with Priority | Page | Cluster | Evidence (actual) | Observed Volume | Revenue Status. Revenue Status defaults to "Blocked: no CRM/product revenue join" unless revenue data is actually present. Add data-derived P0s the template missed if a query family with proven CTR has no commercial page. Correct any template evidence the data contradicts, inline, visibly.
...
15. **Confidence Legend**: renamed in content to "Evidence Volume Tier Legend." State the three metric-specific tier definitions (R7) verbatim, the explicit line that these are volume tiers and not statistical confidence or proof of causality, and the kill-criteria rule.
16. **Ranked Evidence-Priority Actions**: a single table, Tier/Rank Group | Action | Effort | Observed Volume (n) | Depends On | Overlap Note (per R12) | Evidence Volume Tier, consolidating every action recommended anywhere in the report per R10 and R12, ordered by R10's tiered rule. Include the R10 framing statement verbatim: this is priority by evidence tier, category, and effort, not a forecast of business return, and units are not numerically compared across categories. This is the section a reader should be able to act on without reading the rest of the document.
17. **Compliance Self-Check**: the outcome of R13, listed as R1 through R12 (including R7a) each marked Applied or Blocked: <reason>, plus the input-list and section-list completeness checks. This section is what makes the audit's rule-following auditable by someone other than the model that produced it.
...
- Never state a root cause (a WHY) as fact when only WHERE/HOW MUCH data is available (R11). Label it a Hypothesis and name how to validate it.
...
- Never call an event a "standard GA4 event" unless it is genuinely one of GA4's automatically or enhanced-measurement collected events; SEMAI-recommended custom events get their own label.
...
- Never fill an "Expected" or "Kill if" cell with a value that is not sourced from a supplied target, a prior-period benchmark, or a shown mathematical derivation (R7a).
- Never rank attribution-leakage hypotheses by a suspected or estimated size; rank by evidence actually present, or leave unranked (R5).
...
- Presenting a WHERE finding (a page or segment where drop-off is concentrated) as if it were a WHY finding (a root cause) without session replay, user testing, or equivalent qualitative evidence (R11).
